<?php
/**
 * admin/cupones/index.php - Gestión de Descuentos
 */
session_start();
...
// 2. Acciones GET (Borrar / Alternar Estado)
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] === 'delete') {
        ...
        // Cambiar de activo a inactivo y viceversa
...
if (isset($_GET['msg'])) {
    ...
    if($_GET['msg']=='toggled') { $msg='Estado del cupón actualizado.'; $msgType='success'; }
}
...
<body class="bg-slate-50 font-sans flex h-screen overflow-hidden" x-data="{ showModal: false, couponForImage: null }">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div class="xl:col-span-1">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-0">
                        <h2 class="font-bold text-gray-700 uppercase text-sm mb-4 flex items-center gap-2">
                            <i data-lucide="plus-circle" class="w-4 h-4 text-escala-green"></i> Nuevo Cupón
                        </h2>
                        <form method="POST" class="space-y-4">
                            <input type="hidden" name="crear_cupon" value="1">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Nombre Interno (Campaña)</label>
                                <input type="text" name="nombre_interno" required placeholder="Ej. San Valentín" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-escala-green focus:outline-none font-bold text-gray-700">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Código (Se hará mayúsculas)</label>
                                <input type="text" name="codigo" required placeholder="Ej. AMOR2026" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-escala-green focus:outline-none font-black text-escala-dark uppercase tracking-wider" onkeyup="this.value = this.value.toUpperCase();">
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Tipo Oferta</label>
                                    <select name="tipo_descuento" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-escala-green focus:outline-none text-gray-700">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Límite Usos (0 = Infinito)</label>
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Vence (Opcional)</label>
                                    <input type="date" name="fecha_vencimiento" class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-escala-green focus:outline-none text-gray-700">
                                </div>
                            </div>
                            <button type="submit" class="w-full py-3 bg-escala-dark hover:bg-escala-green text-white rounded-xl font-bold uppercase shadow-md transition-all flex justify-center gap-2">
                ...
                <div class="xl:col-span-2">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                         <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <h3 class="font-bold text-gray-700 uppercase text-sm">Listado de Cupones</h3>
                            <span class="text-[10px] font-bold text-gray-400 uppercase">Ordenados por fecha</span>
                        </div>
                        <table class="w-full text-left">
                            <thead class="bg-white text-gray-400 text-[10px] uppercase font-black tracking-wider border-b border-gray-100">
                                <tr>
                            ...
                            <tbody class="divide-y divide-gray-50 text-sm font-medium text-gray-600">
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <p class="font-bold text-gray-800"><?php echo $row['nombre_interno']; ?></p>
                                        <span class="text-xs font-black text-escala-green bg-escala-green/10 px-2 py-0.5 rounded uppercase tracking-wider"><?php echo $row['codigo']; ?></span>
                                    </td>
                                    <td class="px-6 py-4 font-black text-lg text-escala-dark">
                                        <?php echo $row['tipo_descuento'] === 'porcentaje' ? round($row['valor']).'%' : '$'.number_format($row['valor'], 2); ?>
                                    </td>
                                    <td class="px-6 py-4">
                                        <?php 
                                            if ($row['agotado']) {
                                                echo '<span class="badge bg-gray-100 text-gray-500">Agotado</span>';
                                            } elseif ($row['vencido']) {
                                                echo '<span class="badge bg-red-100 text-red-500">Vencido</span>';
                                            } elseif ($row['estado'] === 'activo') {
                                                echo '<span class="badge bg-green-100 text-green-600">Activo</span>';
                                            } else {
                                                echo '<span class="badge bg-yellow-100 text-yellow-600">Pausado</span>';
                                            }
                                        ?>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <a href="index.php?action=toggle&id=<?php echo $row['id']; ?>" class="p-2 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 transition-colors tooltip" title="<?php echo $row['estado']==='activo' ? 'Pausar' : 'Activar'; ?>">
    ...
    <script>
        lucide.createIcons();
        // Función mágica para descargar el DIV como imagen
    ...
    <style>
        .badge { @apply px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide; }
