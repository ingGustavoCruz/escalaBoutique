<?php
/**
 * admin/productos/crear.php - Alta de Productos con Subida Estándar
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sel_inc = isset($_POST['sel_inc']) ? 1 : 0; // Captura del nuevo Toggle
    $indice_portada = isset($_POST['indice_portada']) ? (int)$_POST['indice_portada'] : 0;
    
    $conn->begin_transaction();

    try {
        // A. Insertar Producto Base (Incluyendo sel_inc)
        $stmt = $conn->prepare("INSERT INTO productos (nombre, categoria, precio, descripcion_corta, descripcion_larga, stock, sel_inc, fecha_creacion) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("ssdssii", $nombre, $cat, $precio, $desc_c, $desc_l, $stock_simple, $sel_inc);
        
        if (!$stmt->execute()) throw new Exception("Error en BD al crear producto.");
        $producto_id = $conn->insert_id;

        // B. Procesar Tallas y Stocks
        ...
        // C. Procesar IMÁGENES (Subida directa sin optimización GD)
        if (isset($_FILES['imagenes'])) {
            ...
            for ($i = 0; $i < $total_archivos; $i++) {
                if ($archivos['error'][$i] === UPLOAD_ERR_OK) {
                    ...
                    $nombre_unico = "prod_" . $producto_id . "_" . time() . "_" . $i . "." . $ext;
                    $ruta_destino = "../../imagenes/" . $nombre_unico;
                    $ruta_bd      = "imagenes/" . $nombre_unico;
...
<body class="bg-slate-50 min-h-screen pb-10">
    ...
    <main class="max-w-5xl mx-auto px-6 py-8">
        ...
        <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-3 gap-8" x-data="{ hasVariants: false, isIncorporada: false }">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b pb-4">
                        ...
                        <div class="flex items-center gap-6">
                            <label class="flex items-center cursor-pointer group">
                                <span class="mr-3 text-[10px] font-black text-gray-400 uppercase tracking-widest group-hover:text-escala-green transition-colors">¿Elije Incorporada?</span>
                                <div class="relative">
                                    <input type="checkbox" name="sel_inc" x-model="isIncorporada" class="sr-only">
                                    <div class="w-10 h-5 rounded-full shadow-inner transition-colors" :class="isIncorporada ? 'bg-escala-beige' : 'bg-slate-100'"></div>
                                    <div class="absolute left-1 top-1 bg-white w-3 h-3 rounded-full shadow transition-transform" :class="isIncorporada ? 'translate-x-5' : ''"></div>
                                </div>
                            </label>
                            ...
                            <label class="flex items-center cursor-pointer group">
                                ...
                                <div class="relative">
                                    ...
                                    <div class="w-10 h-5 rounded-full shadow-inner transition-colors" :class="hasVariants ? 'bg-escala-green' : 'bg-slate-100'"></div>
