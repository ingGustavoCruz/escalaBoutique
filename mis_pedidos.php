...
// --- SEGURIDAD ---
// En producción, utiliza la sesión real. Para desarrollo mantenemos el fallback.
$empleado_id = $_SESSION['empleado_id_db'] ?? 1; 

// Datos de la sesión para el encabezado
$empleado_nombre = $_SESSION['usuario_empleado']['nombre'] ?? "Empleado Pruebas";
$empleado_num = $_SESSION['usuario_empleado']['numero'] ?? "EMP001";
...
<body class="min-h-screen bg-slate-50 text-gray-800 font-sans flex flex-col">
    ...
    <header class="bg-escala-green py-4 shadow-lg sticky top-0 z-30 border-b border-escala-dark">
        <div class="max-w-6xl mx-auto px-4 flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <h1 class="font-black uppercase tracking-widest text-lg md:text-xl text-white">MIS PEDIDOS</h1>
            </div>
                        ...
                        <div class="p-5 flex flex-wrap items-center justify-between gap-4 bg-gray-50/50 border-b border-gray-100 cursor-pointer" 
                            ...
                            <div class="flex items-center gap-3">
                                <?php 
                                    ...
                                    $estilo = 'bg-gray-100 text-gray-500'; $icono = 'circle';
                            ...
                            <div class="p-5">
                                <div class="mb-6 p-4 bg-slate-50 rounded-xl border border-gray-100">
                                    <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                        <i data-lucide="calendar-range" class="w-3 h-3 text-escala-beige"></i> Proyección de Descuentos
                                    </h4>
                                    <div class="flex gap-2 h-2 mb-2">
                                        <?php for($i=1; $i<=$p['plazos']; $i++): ?>
                                            <div class="flex-1 rounded-full <?php echo ($estado == 'entregado' || $estado == 'aprobado') ? 'bg-escala-green' : 'bg-gray-200'; ?> opacity-50"></div>
                                        <?php endfor; ?>
                                    </div>
                                    <div class="flex justify-between items-center text-[11px] font-bold">
                                        <span class="text-gray-500">Plan: <span class="text-escala-dark"><?php echo $p['plazos']; ?> quincena(s)</span></span>
                                        <span class="text-escala-green">$<?php echo number_format($p['monto_total'] / $p['plazos'], 2); ?> c/u</span>
                                    </div>
                                </div>

                                <p class="text-xs font-bold text-gray-400 uppercase mb-3 tracking-wider">Productos en este pedido:</p>
                                <div class="space-y-3">
    ...
    <footer class="bg-gray-50 py-8 border-t-2 border-escala-blue mt-auto">
        <div class="max-w-6xl mx-auto px-4 flex flex-col items-center justify-center">
            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] mb-4">Powered By</p>
            <img src="imagenes/KAI_NA.png" alt="KAI Experience" class="h-10 w-auto opacity-60 grayscale hover:grayscale-0 transition-all">
        </div>
    </footer>
