 ...
 * Versión Final: Transacción BD + Notificación + Auditoría + Incorporadas + Envíos Foráneos
 */
session_start();
...
if (!isset($_SESSION['empleado_id_db'])) { 
    echo json_encode(['status' => 'error', 'message' => 'Sesión expirada. Por favor recarga.']); 
    exit; 
}
...
try {
    ...
    foreach ($carrito as $item) {
        ...
        if ($talla) {
            ...
            if (!$stmtUpd->execute()) throw new Exception("Error stock talla."); 
            $stmtUpd->close();
            ...
            if (!$stmtUpd->execute()) throw new Exception("Error stock general."); 
            $stmtUpd->close();
        ...
        if ($incorporada_id) {
            ...
            if ($stmtInc->fetch()) $inc_nombre = $nombre_empresa; 
            $stmtInc->close();
        }
    ...
    for ($i = 1; $i <= $plazos; $i++) {
        ...
        $stmtPagos->bind_param("iiiid", $pedido_id, $empleado_id, $i, $plazos, $monto_final); 
        $stmtPagos->execute(); 
        $stmtPagos->close();
    }
    ...
    $htmlEnvio = $requiere_envio ? "
        ...
        <table style='width: 100%; font-size: 13px; background: #eef2f3; padding: 10px; border-radius: 5px;'>
            <tr><td style='padding-bottom: 5px;'><strong>Recibe:</strong> {$form_envio['nombre_contacto']} ({$form_envio['telefono_contacto']})</td></tr>
    ...
    try {
        if (function_exists('enviarCorreo')) {
            foreach ($destinatarios as $dest) { 
                if (filter_var($dest['email'], FILTER_VALIDATE_EMAIL)) enviarCorreo($dest['email'], $asunto, $mensajeHTML); 
            }
        }
    } catch (Exception $e) { 
        error_log("Error email pedido #$pedido_id"); 
    }
